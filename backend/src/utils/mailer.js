...
const EMAIL_FROM = process.env.EMAIL_FROM || `EDUkinder <${process.env.SMTP_USER || "onboarding@resend.dev"}>`;
...
async function sendEmail({ to, subject, html }) {
  if (!process.env.RESEND_API_KEY) {
    console.log("[EMAIL] Missing RESEND_API_KEY, email not sent.");
...
async function sendInvitationMail(to, token) {
  const invLink = `${APP_URL}/registration?token=${token}`;
